...
plasma:
	sudo apt -y install plasma-desktop dolphin konsole ark kde-spectacle
...
tools:
	sudo apt -y install htop meld guake keepassx retext vim geany glade ghex myrepos baobab restic duplicity deja-dup byobu gnome-tweaks pv fortune cowsay lolcat screenfetch autokey-gtk shutter yubikey-personalization-gui flatpak-builder fio glances rename
